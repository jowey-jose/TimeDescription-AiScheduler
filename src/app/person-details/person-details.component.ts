...
export class PersonDetailsComponent implements OnInit {
  ...
  @ViewChild('breakHourObj',  {static: false})
  public breakHourObj: DialogComponent;
  public activeData: { [key: string]: Object };
  public personData: { [key: string]: Object }[];
  ...
  constructor(public dataService: DataService, public router: Router, private route: ActivatedRoute, ) {
    this.personData = this.dataService.getPersonsData();
    this.specializationData = this.dataService.specialistData;
  }

  ngOnInit() {
    this.dataService.updateActiveItem('persons');
    this.route.params.subscribe((params: any) => this.personId = parseInt(params.id, 10));
    this.personData = this.dataService.getPersonsData();
    this.activeData = this.personData.filter(item => item.Id === this.personId)[0];
    const isDataDiffer: boolean = JSON.stringify(this.activeData) === JSON.stringify(this.dataService.getActivePersonData());
    if (!isDataDiffer) {
      this.dataService.setActivePersonData(this.activeData);
    }
  ...
  onPersonDelete() {
    const filteredData: { [key: string]: Object }[] = this.personData.filter(
      (item: any) => item.Id !== parseInt(this.activeData['Id'] as string, 10));
    this.personData = filteredData;
    this.activeData = this.personData[0];
    this.dataService.setActivePersonData(this.activeData);
    this.dataService.setPersonsData(this.personData);
  }

  onPersonEdit() {
    this.addEditPersonObj.showDetails();
  }
  ...
  onCancelClick() {
    this.breakDays = this.dataService.getActivePersonData()['WorkDays'];
  ...
  onSaveClick() {
    ...
    this.dataService.onUpdateData('WorkDays', workDays, 'person', this.activeData);
    this.breakHourObj.hide();
  }
  ...
  refreshDetails() {
    this.activeData = this.dataService.getActivePersonData();
  }
}
